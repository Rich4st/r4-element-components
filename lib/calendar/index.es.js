...
var FakeComponent = function(_super) {
  __extends$3(FakeComponent2, _super);
...
function createAriaClickAttrs(handler) {
  return __assign$2({ onClick: handler }, createAriaKeyboardAttrs(handler));
}
...
function buildNativeFormattingFunc(standardDateProps, extendedSettings, context) {
  standardDateProps = __assign$2({}, standardDateProps);
  extendedSettings = __assign$2({}, extendedSettings);
  sanitizeSettings(standardDateProps, extendedSettings);
  standardDateProps.timeZone = "UTC";
  var normalFormat = new Intl.DateTimeFormat(context.locale.codes, standardDateProps);
  var zeroFormat;
  if (extendedSettings.omitZeroMinute) {
    var zeroProps = __assign$2({}, standardDateProps);
...
function mergeEventStores(store0, store1) {
  return {
    defs: __assign$2(__assign$2({}, store0.defs), store1.defs),
    instances: __assign$2(__assign$2({}, store0.instances), store1.instances)
  };
}
...
var EVENT_REFINERS = __assign$2(__assign$2(__assign$2({}, EVENT_NON_DATE_REFINERS), EVENT_DATE_REFINERS), { extendedProps: identity });
...
function buildEventRefiners(context) {
  return __assign$2(__assign$2(__assign$2({}, EVENT_UI_REFINERS), EVENT_REFINERS), context.pluginHooks.eventRefiners);
}
function parseEventDef(refined, extra, sourceId, allDay, hasEnd, context) {
  var def = {
    ...
    extendedProps: __assign$2(__assign$2({}, refined.extendedProps || {}), extra)
  };
  for (var _i = 0, _a = context.pluginHooks.eventDefMemberAdders; _i < _a.length; _i++) {
    var memberAdder = _a[_i];
    __assign$2(def, memberAdder(refined));
  }
...
function buildSegCompareObj(seg) {
  ...
  return __assign$2(__assign$2(__assign$2({}, eventDef.extendedProps), eventDef), {
...
function buildDateSpanApi(span, dateEnv) {
  return __assign$2(__assign$2({}, buildRangeApi(span.range, dateEnv, span.allDay)), { allDay: span.allDay });
}
function buildRangeApiWithTimeZone(range, dateEnv, omitTime) {
  return __assign$2(__assign$2({}, buildRangeApi(range, dateEnv, omitTime)), { timeZone: dateEnv.timeZone });
}
...
function triggerDateSelect(selection, pev, context) {
  context.emitter.trigger("select", __assign$2(__assign$2({}, buildDateSpanApiWithContext(selection, context)), { jsEvent: pev ? pev.origEvent : null, view: context.viewApi || context.calendarApi.view }));
}
...
function buildDateSpanApiWithContext(dateSpan, context) {
  ...
  for (var _i = 0, _a = context.pluginHooks.dateSpanTransforms; _i < _a.length; _i++) {
    var transform = _a[_i];
    __assign$2(props, transform(dateSpan, context));
  }
  __assign$2(props, buildDateSpanApi(dateSpan, context.dateEnv));
  return props;
}
...
function applyMutationToEventDef(eventDef, eventConfig, mutation, context) {
  ...
  var copy = __assign$2(__assign$2(__assign$2({}, eventDef), standardProps), { ui: __assign$2(__assign$2({}, eventDef.ui), standardProps.ui) });
  if (mutation.extendedProps) {
    copy.extendedProps = __assign$2(__assign$2({}, copy.extendedProps), mutation.extendedProps);
  }
...
function applyMutationToEventInstance(eventInstance, eventDef, eventConfig, mutation, context) {
  ...
  var copy = __assign$2({}, eventInstance);
  if (forceAllDay) {
...
function buildEventSourceRefiners(context) {
  return __assign$2(__assign$2(__assign$2({}, EVENT_UI_REFINERS), EVENT_SOURCE_REFINERS), context.pluginHooks.eventSourceRefiners);
}
...
var CalendarApi = function() {
  ...
  CalendarApi2.prototype.trigger = function(handlerName) {
    ...
    (_a = this.currentDataManager.emitter).trigger.apply(_a, __spreadArray$1([handlerName], args));
  };
  ...
  EventApi2.prototype.toPlainObject = function(settings) {
    ...
    if (Object.keys(def.extendedProps).length) {
      if (settings.collapseExtendedProps) {
        __assign$2(res, def.extendedProps);
      } else {
...
var RAW_EN_LOCALE = __assign$2(__assign$2({}, MINIMAL_RAW_EN_LOCALE), {
  buttonHints: {
    prev: "Previous $0",
    next: "Next $0",
...
function refineInputs(input) {
  ...
  rawDefs = rawDefs.map(function(rawDef) {
    return __assign$2(__assign$2({}, DEF_DEFAULTS), rawDef);
  });
  return rawDefs;
}
...
function buildNavLinkAttrs(context, dateMarker, viewType, isTabbable) {
  ...
  if (options.navLinks) {
    ...
    return __assign$2({ title: formatWithOrdinals(options.navLinkHint, [dateStr, zonedDate], dateStr), "data-navlink": "" }, isTabbable ? createAriaClickAttrs(handleInteraction) : { onClick: handleInteraction });
  }
...
var ElementScrollController = function(_super) {
  __extends$2(ElementScrollController2, _super);
...
var WindowScrollController = function(_super) {
  __extends$2(WindowScrollController2, _super);
...
var Theme = function() {
  ...
  Theme2.prototype.setIconOverride = function(iconOverrideHash) {
    ...
    if (typeof iconOverrideHash === "object" && iconOverrideHash) {
      iconClassesCopy = __assign$2({}, this.iconClasses);
...
var ScrollResponder = function() {
  function ScrollResponder2(execFunc, emitter, scrollTime, scrollTimeReset) {
    ...
    this.handleScrollRequest = function(request) {
      _this.queuedRequest = __assign$2({}, _this.queuedRequest || {}, request);
      _this.drain();
    };
...
var PureComponent = function(_super) {
  __extends$2(PureComponent2, _super);
...
var BaseComponent = function(_super) {
  __extends$2(BaseComponent2, _super);
...
function addPropsEquality(propEquality) {
  ...
  __assign$2(hash, propEquality);
  this.prototype.propEquality = hash;
}
function addStateEquality(stateEquality) {
  var hash = Object.create(this.prototype.stateEquality);
  __assign$2(hash, stateEquality);
...
var DateComponent = function(_super) {
  __extends$2(DateComponent2, _super);
...
function combineHooks(hooks0, hooks1) {
  return {
    ...
    eventRefiners: __assign$2(__assign$2({}, hooks0.eventRefiners), hooks1.eventRefiners),
    eventDefMemberAdders: hooks0.eventDefMemberAdders.concat(hooks1.eventDefMemberAdders),
    eventSourceRefiners: __assign$2(__assign$2({}, hooks0.eventSourceRefiners), hooks1.eventSourceRefiners),
    ...
    views: __assign$2(__assign$2({}, hooks0.views), hooks1.views),
    ...
    themeClasses: __assign$2(__assign$2({}, hooks0.themeClasses), hooks1.themeClasses),
    ...
    optionChangeHandlers: __assign$2(__assign$2({}, hooks0.optionChangeHandlers), hooks1.optionChangeHandlers),
    scrollGridImpl: hooks1.scrollGridImpl || hooks0.scrollGridImpl,
    contentTypeHandlers: __assign$2(__assign$2({}, hooks0.contentTypeHandlers), hooks1.contentTypeHandlers),
    listenerRefiners: __assign$2(__assign$2({}, hooks0.listenerRefiners), hooks1.listenerRefiners),
    optionRefiners: __assign$2(__assign$2({}, hooks0.optionRefiners), hooks1.optionRefiners),
    propSetHandlers: __assign$2(__assign$2({}, hooks0.propSetHandlers), hooks1.propSetHandlers)
  };
}
var StandardTheme = function(_super) {
  __extends$2(StandardTheme2, _super);
...
function buildViewDef(viewType, hash, defaultConfigs, overrideConfigs) {
  ...
  return {
    ...
    defaults: __assign$2(__assign$2({}, superDef ? superDef.defaults : {}), defaultConfig ? defaultConfig.rawOptions : {}),
    overrides: __assign$2(__assign$2({}, superDef ? superDef.overrides : {}), overrideConfig ? overrideConfig.rawOptions : {})
  };
}
var RenderHook = function(_super) {
  __extends$2(RenderHook2, _super);
...
function ContentHook(props) {
  return createElement(CustomContentRenderContext.Consumer, null, function(renderId) {
    return createElement(ContentHookInner, __assign$2({ renderId }, props));
  });
}
var ContentHookInner = function(_super) {
  __extends$2(ContentHookInner2, _super);
  ...
  ContentHookInner2.prototype.renderInnerContent = function() {
    ...
    if (!customContentInfo || customContentInfo.contentKey !== meta.contentKey) {
      ...
      if (meta.contentKey) {
        customContentInfo = this.customContentInfo = __assign$2({ contentKey: meta.contentKey, contentVal: innerContent[meta.contentKey] }, meta.buildLifecycleFuncs());
      }
...
var MountHook = function(_super) {
  ...
  MountHook2.prototype.componentDidMount = function() {
    ...
    if (callback) {
      callback(__assign$2(__assign$2({}, this.props.hookProps), { el: this.rootEl }));
    }
  };
  ...
  return function(viewProps) {
    return createElement(ViewContextType.Consumer, null, function(context) {
      return createElement(ViewRoot, { viewSpec: context.viewSpec }, function(viewElRef, viewClassNames) {
        var hookProps = __assign$2(__assign$2({}, viewProps), { nextDayThreshold: context.options.nextDayThreshold });
...
function buildViewSpec(viewDef, overrideConfigs, optionOverrides, dynamicOptionOverrides, localeDefaults) {
  ...
  return {
    ...
    optionOverrides: __assign$2(__assign$2({}, singleUnitOverrides), viewDef.overrides),
...
function reduceDynamicOptionOverrides(dynamicOptionOverrides, action) {
  ...
  switch (action.type) {
    case "SET_OPTION":
      return __assign$2(__assign$2({}, dynamicOptionOverrides), (_a = {}, _a[action.optionName] = action.rawOptionValue, _a));
    default:
...
function addSources(eventSourceHash, sources, fetchRange, context) {
  ...
  return __assign$2(__assign$2({}, eventSourceHash), hash);
}
...
function fetchSource(eventSource, fetchRange, isRefetch, context) {
  ...
  return __assign$2(__assign$2({}, eventSource), { isFetching: true, latestFetchId: fetchId });
}
function receiveResponse(sourceHash, sourceId, fetchId, fetchRange) {
  var _a;
  var eventSource = sourceHash[sourceId];
  if (eventSource && fetchId === eventSource.latestFetchId) {
    return __assign$2(__assign$2({}, sourceHash), (_a = {}, _a[sourceId] = __assign$2(__assign$2({}, eventSource), { isFetching: false, fetchRange }), _a));
  }
  return sourceHash;
}
...
function rezoneEventStoreDates(eventStore, oldDateEnv, newDateEnv) {
  ...
  var instances = mapHash(eventStore.instances, function(instance) {
    ...
    return __assign$2(__assign$2({}, instance), { range: {
...
function buildRequestParams(meta, range, context) {
  ...
  __assign$2(params, customRequestParams);
...
function handleDateProfile(dateProfile, context) {
  context.emitter.trigger("datesSet", __assign$2(__assign$2({}, buildRangeApiWithTimeZone(dateProfile.activeRange, context.dateEnv)), { view: context.viewApi }));
}
...
var CalendarDataManager = function() {
  function CalendarDataManager2(props) {
    ...
    var contextAndState = __assign$2(__assign$2({}, calendarContext), initialState);
    for (var _b = 0, _c = optionsData.pluginHooks.reducers; _b < _c.length; _b++) {
      var reducer = _c[_b];
      __assign$2(initialState, reducer(null, null, contextAndState));
    }
  ...
  CalendarDataManager2.prototype.resetOptions = function(optionOverrides, append) {
    var props = this.props;
    props.optionOverrides = append ? __assign$2(__assign$2({}, props.optionOverrides), optionOverrides) : optionOverrides;
  ...
  CalendarDataManager2.prototype._handleAction = function(action) {
    ...
    var contextAndState = __assign$2(__assign$2({}, calendarContext), newState);
    for (var _i = 0, _c = optionsData.pluginHooks.reducers; _i < _c.length; _i++) {
      var reducer = _c[_i];
      __assign$2(newState, reducer(state, action, contextAndState));
    }
  ...
  CalendarDataManager2.prototype.updateData = function() {
    ...
    var data = this.data = __assign$2(__assign$2(__assign$2({ viewTitle: this.buildTitle(state.dateProfile, currentViewData.options, optionsData.dateEnv), calendarApi: props.calendarApi, dispatch: this.dispatch, emitter: this.emitter, getCurrentData: this.getCurrentData }, optionsData), currentViewData), state);
  ...
  CalendarDataManager2.prototype.processRawCalendarOptions = function(optionOverrides, dynamicOptionOverrides) {
    ...
    var refiners = this.currentCalendarOptionsRefiners = __assign$2(__assign$2(__assign$2(__assign$2(__assign$2({}, BASE_OPTION_REFINERS), CALENDAR_LISTENER_REFINERS), CALENDAR_OPTION_REFINERS), pluginHooks.listenerRefiners), pluginHooks.optionRefiners);
    var extra = {};
  ...
  CalendarDataManager2.prototype.processRawViewOptions = function(viewSpec, pluginHooks, localeDefaults, optionOverrides, dynamicOptionOverrides) {
    ...
    var refiners = __assign$2(__assign$2(__assign$2(__assign$2(__assign$2(__assign$2({}, BASE_OPTION_REFINERS), CALENDAR_LISTENER_REFINERS), CALENDAR_OPTION_REFINERS), VIEW_OPTION_REFINERS), pluginHooks.listenerRefiners), pluginHooks.optionRefiners);
    var refined = {};
...
(function(_super) {
  __extends$2(CalendarDataProvider, _super);
...
var SegHierarchy = function() {
  ...
  SegHierarchy2.prototype.splitEntry = function(entry, barrier, hiddenEntries) {
    ...
    if (partCnt) {
      hiddenEntries.push.apply(hiddenEntries, __spreadArray$1([{
        index: entry.index,
        thickness: entry.thickness,
  ...
  SegHierarchy2.prototype.toRects = function() {
    ...
    for (var level = 0; level < levelCnt; level += 1) {
      ...
      for (var _i = 0, entries_1 = entries; _i < entries_1.length; _i++) {
        ...
        rects.push(__assign$2(__assign$2({}, entry), { levelCoord }));
      }
    }
    return rects;
  };
...
var ToolbarSection = function(_super) {
  __extends$2(ToolbarSection2, _super);
  ...
  ToolbarSection2.prototype.render = function() {
    ...
    return createElement.apply(void 0, __spreadArray$1(["div", { className: "fc-toolbar-chunk" }], children));
  };
  ToolbarSection2.prototype.renderWidgetGroup = function(widgetGroup) {
    ...
    if (children.length > 1) {
      ...
      return createElement.apply(void 0, __spreadArray$1(["div", { className: groupClassName }], children));
    }
    return children[0];
  };
...
var ViewContainer = function(_super) {
  __extends$2(ViewContainer2, _super);
...
var EventClicking = function(_super) {
  __extends$2(EventClicking2, _super);
...
var EventHovering = function(_super) {
  __extends$2(EventHovering2, _super);
...
var CalendarContent = function(_super) {
  __extends$2(CalendarContent2, _super);
  ...
  CalendarContent2.prototype.render = function() {
    ...
    return createElement(ViewContextType.Provider, { value: viewContext }, toolbarConfig.header && createElement(Toolbar, __assign$2({ ref: this.headerRef, extraClassName: "fc-header-toolbar", model: toolbarConfig.header, titleId: viewLabelId }, toolbarProps)), createElement(ViewContainer, { liquid: viewVGrow, height: viewHeight, aspectRatio: viewAspectRatio, labeledById: viewLabelId }, this.renderView(props), this.buildAppendContent()), toolbarConfig.footer && createElement(Toolbar, __assign$2({ ref: this.footerRef, extraClassName: "fc-footer-toolbar", model: toolbarConfig.footer, titleId: "" }, toolbarProps)));
  };
  ...
  CalendarContent2.prototype.buildAppendContent = function() {
    ...
    return createElement.apply(void 0, __spreadArray$1([Fragment, {}], children));
  };
  CalendarContent2.prototype.renderView = function(props) {
    ...
    for (var _i = 0, transformers_1 = transformers; _i < transformers_1.length; _i++) {
      var transformer = transformers_1[_i];
      __assign$2(viewProps, transformer.transform(viewProps, props));
    }
    var ViewComponent = viewSpec.component;
    return createElement(ViewComponent, __assign$2({}, viewProps));
  };
...
var CalendarRoot = function(_super) {
  __extends$2(CalendarRoot2, _super);
...
var TableDateCell = function(_super) {
  __extends$2(TableDateCell2, _super);
  ...
  TableDateCell2.prototype.render = function() {
    ...
    var hookProps = __assign$2(__assign$2(__assign$2({ date: dateEnv.toDate(date), view: viewApi }, props.extraHookProps), { text }), dayMeta);
    return createElement(RenderHook, { hookProps, classNames: options.dayHeaderClassNames, content: options.dayHeaderContent, defaultContent: renderInner$1, didMount: options.dayHeaderDidMount, willUnmount: options.dayHeaderWillUnmount }, function(rootElRef, customClassNames, innerElRef, innerContent) {
      return createElement("th", __assign$2({ ref: rootElRef, role: "columnheader", className: classNames.concat(customClassNames).join(" "), "data-date": !dayMeta.isDisabled ? formatDayString(date) : void 0, colSpan: props.colSpan }, props.extraDataAttrs), createElement("div", { className: "fc-scrollgrid-sync-inner" }, !dayMeta.isDisabled && createElement("a", __assign$2({ ref: innerElRef, className: [
...
var TableDowCell = function(_super) {
  __extends$2(TableDowCell2, _super);
  ...
  TableDowCell2.prototype.render = function() {
    ...
    var text = dateEnv.format(date, props.dayHeaderFormat);
    var hookProps = __assign$2(__assign$2(__assign$2(__assign$2({
      date
    }, dateMeta), { view: viewApi }), props.extraHookProps), { text });
    return createElement(RenderHook, { hookProps, classNames: options.dayHeaderClassNames, content: options.dayHeaderContent, defaultContent: renderInner$1, didMount: options.dayHeaderDidMount, willUnmount: options.dayHeaderWillUnmount }, function(rootElRef, customClassNames, innerElRef, innerContent) {
      return createElement("th", __assign$2({ ref: rootElRef, role: "columnheader", className: classNames.concat(customClassNames).join(" "), colSpan: props.colSpan }, props.extraDataAttrs), createElement("div", { className: "fc-scrollgrid-sync-inner" }, createElement("a", { "aria-label": dateEnv.format(date, WEEKDAY_FORMAT), className: [
        "fc-col-header-cell-cushion",
        props.isSticky ? "fc-sticky" : ""
...
var Slicer = function() {
  ...
  Slicer2.prototype.sliceProps = function(props, dateProfile, nextDayThreshold, context) {
    ...
    var eventSegs = this.sliceEventStore.apply(this, __spreadArray$1([props.eventStore, eventUiBases, dateProfile, nextDayThreshold], extraArgs));
    return {
      dateSelectionSegs: this.sliceDateSelection.apply(this, __spreadArray$1([props.dateSelection, eventUiBases, context], extraArgs)),
      businessHourSegs: this.sliceBusinessHours.apply(this, __spreadArray$1([props.businessHours, dateProfile, nextDayThreshold, context], extraArgs)),
      fgEventSegs: eventSegs.fg,
      bgEventSegs: eventSegs.bg,
      eventDrag: this.sliceEventDrag.apply(this, __spreadArray$1([props.eventDrag, eventUiBases, dateProfile, nextDayThreshold], extraArgs)),
      eventResize: this.sliceEventResize.apply(this, __spreadArray$1([props.eventResize, eventUiBases, dateProfile, nextDayThreshold], extraArgs)),
      eventSelection: props.eventSelection
    };
  };
  Slicer2.prototype.sliceNowDate = function(date, context) {
    ...
    return this._sliceDateSpan.apply(this, __spreadArray$1([
  ...
  Slicer2.prototype._sliceBusinessHours = function(businessHours, dateProfile, nextDayThreshold, context) {
    ...
    return this._sliceEventStore.apply(this, __spreadArray$1([
  ...
  Slicer2.prototype._sliceDateSpan = function(dateSpan, eventUiBases, context) {
    ...
    var segs = this.sliceRange.apply(this, __spreadArray$1([dateSpan.range], extraArgs));
  ...
  Slicer2.prototype.sliceEventRange = function(eventRange, extraArgs) {
    ...
    var segs = this.sliceRange.apply(this, __spreadArray$1([dateRange], extraArgs));
...
function isNewPropsValid(newProps, context) {
  ...
  var props = __assign$2({ businessHours: calendarState.businessHours, dateSelection: "", eventStore: calendarState.eventStore, eventUiBases: calendarState.eventUiBases, eventSelection: "", eventDrag: null, eventResize: null }, newProps);
...
function isInteractionPropsValid(state, context, dateSpanMeta, filterConfig) {
  ...
  for (var subjectInstanceId in subjectInstances) {
    ...
    for (var _i = 0, _a = subjectConfig.allows; _i < _a.length; _i++) {
      ...
      var subjectDateSpan = __assign$2(__assign$2({}, dateSpanMeta), { range: subjectInstance.range, allDay: subjectDef.allDay });
...
function isDateSelectionPropsValid(state, context, dateSpanMeta, filterConfig) {
  ...
  for (var _i = 0, _a = selectionConfig.allows; _i < _a.length; _i++) {
    ...
    var fullDateSpan = __assign$2(__assign$2({}, dateSpanMeta), selection);
...
function renderMicroColGroup(cols, shrinkWidth) {
  ...
  return createElement.apply(void 0, __spreadArray$1(["colgroup", {}], colNodes));
}
...
var SimpleScrollGrid = function(_super) {
  __extends$2(SimpleScrollGrid2, _super);
  function SimpleScrollGrid2() {
    ...
    _this.handleSizing = function() {
      _this.setState(__assign$2({ shrinkWidth: _this.computeShrinkWidth() }, _this.computeScrollerDims()));
    };
    return _this;
  }
  SimpleScrollGrid2.prototype.render = function() {
    ...
    }, Boolean(!isBuggy && headSectionNodes.length) && createElement.apply(void 0, __spreadArray$1(["thead", roleAttrs], headSectionNodes)), Boolean(!isBuggy && bodySectionNodes.length) && createElement.apply(void 0, __spreadArray$1(["tbody", roleAttrs], bodySectionNodes)), Boolean(!isBuggy && footSectionNodes.length) && createElement.apply(void 0, __spreadArray$1(["tfoot", roleAttrs], footSectionNodes)), isBuggy && createElement.apply(void 0, __spreadArray$1(__spreadArray$1(__spreadArray$1(["tbody", roleAttrs], headSectionNodes), bodySectionNodes), footSectionNodes)));
  };
...
var StandardEvent = function(_super) {
  __extends$2(StandardEvent2, _super);
  ...
  StandardEvent2.prototype.render = function() {
    ...
    return createElement(EventRoot, { seg, timeText, disableDragging: props.disableDragging, disableResizing: props.disableResizing, defaultContent: props.defaultContent || renderInnerContent$1, isDragging: props.isDragging, isResizing: props.isResizing, isDateSelecting: props.isDateSelecting, isSelected: props.isSelected, isPast: props.isPast, isFuture: props.isFuture, isToday: props.isToday }, function(rootElRef, classNames, innerElRef, innerContent, hookProps) {
      return createElement("a", __assign$2({ className: props.extraClassNames.concat(classNames).join(" "), style: {
...
var DayCellContent = function(_super) {
  __extends$2(DayCellContent2, _super);
...
function refineDayCellHookProps(raw) {
  ...
  return __assign$2(__assign$2(__assign$2({ date: dateEnv.toDate(date), view: raw.viewApi }, dayMeta), { dayNumberText: raw.showDayNumber ? dateEnv.format(date, DAY_NUM_FORMAT) : "" }), raw.extraProps);
}
var DayCellRoot = function(_super) {
  __extends$2(DayCellRoot2, _super);
...
var MorePopover = function(_super) {
  __extends$2(MorePopover2, _super);
  ...
  MorePopover2.prototype.queryHit = function(positionLeft, positionTop, elWidth, elHeight) {
    ...
    if (positionLeft >= 0 && positionLeft < elWidth && positionTop >= 0 && positionTop < elHeight) {
      return {
        dateProfile: props.dateProfile,
        dateSpan: __assign$2({ allDay: true, range: {
...
var MoreLinkRoot = function(_super) {
  __extends$2(MoreLinkRoot2, _super);
      ...
      if (_this.isRendering) {
        ...
        flushSync(function() {
          render(createElement(CalendarRoot, { options: currentData_1.calendarOptions, theme: currentData_1.theme, emitter: currentData_1.emitter }, function(classNames, height, isHeightAuto, forPrint) {
            ...
            return createElement(CustomContentRenderContext.Provider, { value: _this.customContentRenderId }, createElement(CalendarContent, __assign$3({ isHeightAuto, forPrint }, currentData_1)));
          }), _this.el);
        });
...
var TableCellTop = function(_super) {
  __extends$1(TableCellTop2, _super);
  ...
  TableCellTop2.prototype.render = function() {
    ...
    return createElement(DayCellContent, { date: props.date, dateProfile: props.dateProfile, todayRange: props.todayRange, showDayNumber: props.showDayNumber, extraHookProps: props.extraHookProps, defaultContent: renderTopInner }, function(innerElRef, innerContent) {
      return (innerContent || props.forceDayTop) && createElement("div", { className: "fc-daygrid-day-top", ref: innerElRef }, createElement("a", __assign$1({ id: props.dayNumberId, className: "fc-daygrid-day-number" }, navLinkAttrs), innerContent || createElement(Fragment, null, "\xA0")));
    });
  };
...
var TableBlockEvent = function(_super) {
  __extends$1(TableBlockEvent2, _super);
  function TableBlockEvent2() {
    return _super !== null && _super.apply(this, arguments) || this;
  }
  TableBlockEvent2.prototype.render = function() {
    var props = this.props;
    return createElement(StandardEvent, __assign$1({}, props, { extraClassNames: ["fc-daygrid-event", "fc-daygrid-block-event", "fc-h-event"], defaultTimeFormat: DEFAULT_TABLE_EVENT_TIME_FORMAT, defaultDisplayEventEnd: props.defaultDisplayEventEnd, disableResizing: !props.seg.eventRange.def.allDay }));
  };
  return TableBlockEvent2;
}(BaseComponent);
var TableListItemEvent = function(_super) {
  __extends$1(TableListItemEvent2, _super);
  ...
  TableListItemEvent2.prototype.render = function() {
    ...
    return createElement(EventRoot, { seg: props.seg, timeText, defaultContent: renderInnerContent, isDragging: props.isDragging, isResizing: false, isDateSelecting: false, isSelected: props.isSelected, isPast: props.isPast, isFuture: props.isFuture, isToday: props.isToday }, function(rootElRef, classNames, innerElRef, innerContent) {
      return createElement("a", __assign$1({ className: ["fc-daygrid-event", "fc-daygrid-dot-event"].concat(classNames).join(" "), ref: rootElRef }, getSegAnchorAttrs(props.seg, context)), innerContent);
    });
  };
...
var TableCellMoreLink = function(_super) {
  __extends$1(TableCellMoreLink2, _super);
  ...
  TableCellMoreLink2.prototype.render = function() {
    ...
    return createElement(MoreLinkRoot, { dateProfile: props.dateProfile, todayRange: props.todayRange, allDayDate: props.allDayDate, moreCnt: props.moreCnt, allSegs, hiddenSegs: invisibleSegs, alignmentElRef: props.alignmentElRef, alignGridTop: props.alignGridTop, extraDateSpan: props.extraDateSpan, popoverContent: function() {
      ...
      return createElement(Fragment, null, allSegs.map(function(seg) {
        ...
        } }, hasListItemDisplay(seg) ? createElement(TableListItemEvent, __assign$1({ seg, isDragging: false, isSelected: instanceId === props.eventSelection, defaultDisplayEventEnd: false }, getSegMeta(seg, props.todayRange))) : createElement(TableBlockEvent, __assign$1({ seg, isDragging: false, isResizing: false, isDateSelecting: false, isSelected: instanceId === props.eventSelection, defaultDisplayEventEnd: false }, getSegMeta(seg, props.todayRange))));
      }));
    } }, function(rootElRef, classNames, innerElRef, innerContent, handleClick, title, isExpanded, popoverId) {
      return createElement("a", __assign$1({ ref: rootElRef, className: ["fc-daygrid-more-link"].concat(classNames).join(" "), title, "aria-expanded": isExpanded, "aria-controls": popoverId }, createAriaClickAttrs(handleClick)), innerContent);
    });
  };
...
function resliceSeg(seg, spanStart, spanEnd, cells) {
  ...
  return __assign$1(__assign$1({}, seg), { firstCol: spanStart, lastCol: spanEnd - 1, eventRange: {
    def: eventRange.def,
    ui: __assign$1(__assign$1({}, eventRange.ui), { durationEditable: false }),
    instance: eventRange.instance,
    range: slicedRange
  }, isStart: seg.isStart && slicedRange.start.valueOf() === origRange.start.valueOf(), isEnd: seg.isEnd && slicedRange.end.valueOf() === origRange.end.valueOf() });
}
var DayGridSegHierarchy = function(_super) {
  __extends$1(DayGridSegHierarchy2, _super);
    ...
    if (this.hiddenConsumes && touchingEntry) {
      ...
      if (!forceHidden[touchingEntryId]) {
        if (this.allowReslicing) {
          var placeholderEntry = __assign$1(__assign$1({}, touchingEntry), { span: intersectSpans(touchingEntry.span, entry.span) });
...
var TableRow = function(_super) {
    ...
    if (framePositions) {
      for (var _i = 0, segPlacements_1 = segPlacements; _i < segPlacements_1.length; _i++) {
        ...
        nodes.push(createElement("div", { className: "fc-daygrid-event-harness" + (isAbsolute ? " fc-daygrid-event-harness-abs" : ""), key, ref: isMirror ? null : this.segHarnessRefs.createRef(key), style: {
          ...
          left,
          right
        } }, hasListItemDisplay(seg) ? createElement(TableListItemEvent, __assign$1({ seg, isDragging, isSelected: instanceId === eventSelection, defaultDisplayEventEnd }, getSegMeta(seg, todayRange))) : createElement(TableBlockEvent, __assign$1({ seg, isDragging, isResizing, isDateSelecting, isSelected: instanceId === eventSelection, defaultDisplayEventEnd }, getSegMeta(seg, todayRange)))));
      }
    }
    return nodes;
  };
  TableRow2.prototype.renderFillSegs = function(segs, fillType) {
    ...
    if (framePositions) {
      for (var _i = 0, segs_1 = segs; _i < segs_1.length; _i++) {
        ...
        nodes.push(createElement("div", { key: buildEventRangeKey(seg.eventRange), className: "fc-daygrid-bg-harness", style: leftRightCss }, fillType === "bg-event" ? createElement(BgEvent, __assign$1({ seg }, getSegMeta(seg, todayRange))) : renderFill(fillType)));
      }
    }
  ...
  TableRow2.prototype.updateSizing = function(isExternalSizingChange) {
    ...
    if (!props.forPrint && props.clientWidth !== null) {
      ...
      this.setState({
        eventInstanceHeights: __assign$1(__assign$1({}, oldInstanceHeights), newInstanceHeights),
  ...
  Table2.prototype.queryHit = function(positionLeft, positionTop) {
    ...
    if (row != null && col != null) {
      ...
      return {
        ...
        dateSpan: __assign$1({ range: this.getCellRange(row, col), allDay: true }, cell.extraDateSpan),
...
var DayTableSlicer = function(_super) {
  __extends$1(DayTableSlicer2, _super);
...
var DayTableView = function(_super) {
  __extends$1(DayTableView2, _super);
...
var TableDateProfileGenerator = function(_super) {
  __extends$1(TableDateProfileGenerator2, _super);
...
var calendar = /* @__PURE__ */ _export_sfc(_sfc_main, [["__scopeId", "data-v-a3452512"]]);
var index = {
  install(app) {
